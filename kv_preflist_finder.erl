-module(kv_preflist_finder).

%% Note that this should be run from `riak escript`, not `riak-cs escript`

%% script_main is where the action is
script_main(_LocalNode, TargetNode, _Cookie, [KVBucketIn, KVKeyIn, NValueIn]) ->
    FindPreflist = fun(KVBucket, KVKey, NValue) ->
        BKey = {KVBucket,KVKey},
    ...
    io:format("Connecting to ~p to find preflist for KV identifier pair ~s ~s~n",[TargetNode, KVBucketIn, KVKeyIn]),
    io:format("~p~n",[rpc:call(TargetNode, erlang, apply, [FindPreflist, [KVBucketIn, KVKeyIn, NValueIn]], infinity)]);
script_main(_LocalNode, _TargetNode, _Cookie, _) ->
    usage().

usage() ->
    io:format("Usage: riak escript <path/to/kv_preflist_finder.erl> [options] <KVBucket> <KVKey> <NValue>~n"),
    io:format("  Options:~n"),
